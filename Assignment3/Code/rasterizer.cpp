...
void rst::rasterizer::rasterize_triangle(const Triangle& t, const std::array<Eigen::Vector3f, 3>& view_pos) 
{
    ...
    float a1 = std::min({v[0].x(), v[1].x(), v[2].x()});
    float a2 = std::max({v[0].x(), v[1].x(), v[2].x()});
    float b1 = std::min({v[0].y(), v[1].y(), v[2].y()});
    float b2 = std::max({v[0].y(), v[1].y(), v[2].y()});

    int min_x = std::floor(a1);
    int max_x = std::ceil(a2);
    int min_y = std::floor(b1);
    int max_y = std::ceil(b2);
